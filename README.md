...
### Preparatory Step — Prompt-Engineered Markdown Templates
...
> **📝 Note for first-time users:** The above files are the current set of markdown templates included in this repository. Each file contains a pre-written prompt that tells Claude what to generate. You are encouraged to customize these templates to match your team's specific needs — for example, adjusting the constraints, adding domain-specific terminology, or modifying the output structure to align with your organization's documentation standards.
...
> **What is tmux?** tmux (terminal multiplexer) is a tool that lets you split your terminal into multiple panes within a single window — allowing you to see all three agents running and communicating with each other in real time, side by side.

#### The SWARM_CONTEXT.md File

Before swarm execution, a `SWARM_CONTEXT.md` file is created to serve as the **shared instruction set** for all three agents. It defines each agent's responsibilities, their communication protocol, and the exact workflow for completing a user story.
